...
def fetch_fund_estimate(code: str):
    ...
    参数：基金代码，如 014611
    返回：dict 包含 fundcode, name, gsz, gzzl, gztime, dwjz, jzrq 等
    """
...
def push_wechat(title: str, content: str):
    ...
    if not token:
        raise RuntimeError("PushPlus Token 未配置，请编辑 config.json 填写 pushplus_token")
...
def main():
    ...
    funds = cfg.get("funds", [])
    if not funds:
        print("⚠️ 请在 config.json 中添加基金代码，示例: ['014611', '000001']")
        return
    ...
    if result.get("code") == 200:
        ...
        print("  2. Token 正确填写在 config.json")
    else:
